...
const TOPICS721 = (0, helper_1.getTopics)(Events721_json_1.default);
const TOPICS_20 = [
    ...TOPICS.Transfer,
    ...TOPICS.Approval,
];
...
class Resource {
    ...
    updateAssets({ updateAssets, account, logs }) {
        if (!account) {
            ...
            for (const log of parsedLogs) {
                    ...
                    if (TOPICS.Transfer.includes(log.topics[0])) {
                        ...
                        if (value) {
                            if (!assets[20].balance[token]) {
                                assets[20].balance[token] = ethers_1.BigNumber.from(0);
                            }
                            if (to === account) {
                ...
                if (TOPICS_1155.includes(log.topics[0])) {
                    if (TOPICS.TransferSingle.includes(log.topics[0])) {
                        ...
                        if (!assets[1155].balance[balanceKey]) {
                            assets[1155].balance[balanceKey] = ethers_1.BigNumber.from(0);
                        }
                        if (to === account) {
                    ...
                    if (TOPICS.ApprovalForAll.includes(log.topics[0])) {
                        ...
                        if (owner === account) {
                            ...
                            assets[1155].allowance[allowanceKey] = approved ? ethers_1.BigNumber.from(ethers_1.ethers.constants.MaxInt256) : ethers_1.BigNumber.from(0);
                        }
                    }
                }
                // ERC-721
                if (TOPICS_721.includes(log.topics[0])) {
                    if (TOPICS.Transfer.includes(log.topics[0]) && log.args?.tokenId) {
                        ...
                        if (to === account) {
                            assets[721].balance[balanceKey] = ethers_1.BigNumber.from(1);
                        }
                        if (from === account) {
                    ...
                    if (TOPICS.Approval.includes(log.topics[0]) && log.args?.tokenId) {
                        ...
                        if (owner === account) {
                            ...
                            assets[721].allowance[allowanceKey] = ethers_1.BigNumber.from(1);
                        }
                    }
                    if (TOPICS.ApprovalForAll.includes(log.topics[0])) {
                        const { owner, operator, approved } = log.args;
                        if (owner === account) {
                            ...
                            assets[721].allowanceALl[allowanceAllKey] = approved ? ethers_1.BigNumber.from(ethers_1.ethers.constants.MaxInt256) : ethers_1.BigNumber.from(0);
                        }
                    }
                }
            }
    ...
    async getNewResource(account, playMode) {
        try {
            ...
            return await provider
                ...
                .then(async ([swapLogs, ddlTokenTransferLogs, transferLogs, bnaLogs, allLogs]) => {
                const result = {
                    pools: {},
                    tokens: [],
                    swapLogs: [],
